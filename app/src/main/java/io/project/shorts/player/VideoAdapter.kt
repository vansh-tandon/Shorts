...
import io.project.shorts.databinding.ListVideoBinding
import io.project.shorts.models.current.PostsItem

class VideoAdapter(
    var context: Context,
    var videos: List<PostsItem?>?,
    ...
    //    holder.setVideoPath(model.url)
        holder.binding.name.text = model!!.creator!!.name
        holder.binding.handle.text= model.creator!!.handle
        holder.setVideoPath(model.submission!!.mediaUrl!!)
        holder.binding.imageViewOptionLikeTitle.text= model.reaction!!.count.toString()
        holder.binding.imageViewOptionCommentTitle.text= model.comment!!.count.toString()
